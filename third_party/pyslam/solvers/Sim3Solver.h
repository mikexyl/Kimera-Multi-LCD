...
class Sim3SolverInput {
 public:
  ...
  std::vector<float> mvSigmaSquare1;
  std::vector<float> mvSigmaSquare2;
  Eigen::Matrix3f K1;
  Eigen::Matrix3f Rcw1;
  Eigen::Vector3f tcw1;
  Eigen::Matrix3f K2;
...
// Estimates the Sim3 transformation between two sets of 3D points
// back-projected from two camera frames. Inliers are evaluated using
// bidirectional reprojection error.
class Sim3Solver {
 public:
  ...
  Eigen::Matrix4f find(std::vector<uint8_t>& vbInliers12,
                       int& nInliers,
                       bool& bConverged);

  Eigen::Matrix4f iterate(const int nIterations,
                          bool& bNoMore,
